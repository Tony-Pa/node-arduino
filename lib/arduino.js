...
const OPC_SET_LIGHT_RELAY  = 0x0C;
...
class Board {
    ...
    setLightRelay(pin, val) {
        this.sp.write(new Buffer([OPC_SET_LIGHT_RELAY, pin, val]));
    }
    ...
    toggleRelays(pins) {
        this.sp.write(new Buffer([OPC_TOGGLE_RELAYS, this.LOW, pins.length, ...pins]));

        setTimeout(() => {
            this.sp.write(new Buffer([OPC_TOGGLE_RELAYS, this.HIGH, pins.length, ...pins]));
        }, 200);
    }
